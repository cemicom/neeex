...
import net.mcreator.basicrpgmod.block.BlockMyticSand;
import net.mcreator.basicrpgmod.ElementsBasicRPGmod;

import java.util.Random;

@ElementsBasicRPGmod.ModElement.Tag
public class BiomeMyticDesert extends ElementsBasicRPGmod.ModElement {
	@GameRegistry.ObjectHolder("basicrpgmod:myticdesert")
	public static final BiomeGenCustom biome = null;
	public BiomeMyticDesert(ElementsBasicRPGmod instance) {
		super(instance, 99);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			...
			topBlock = BlockMyticSand.block.getDefaultState();
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockMyticSand.block.getDefaultState().getBlock()
							|| ground == BlockMyticStone.block.getDefaultState().getBlock())
							&& (ground2 == BlockMyticSand.block.getDefaultState().getBlock()
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == BlockMyticSand.block.getDefaultState().getBlock()
