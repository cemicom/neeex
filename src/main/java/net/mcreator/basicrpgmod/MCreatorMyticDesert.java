...
public class MCreatorMyticDesert extends Elementsbasicrpgmod.ModElement {
	@GameRegistry.ObjectHolder("basicrpgmod:myticdesert")
	public static final BiomeGenCustom biome = null;

	public MCreatorMyticDesert(Elementsbasicrpgmod instance) {
		super(instance, 99);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Mytic Desert").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-65281).setHeightVariation(0.2F)
					.setTemperature(0.5F));
			setRegistryName("myticdesert");
			topBlock = MCreatorMyticSand.block.getDefaultState();
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == MCreatorMyticSand.block.getDefaultState().getBlock() || ground == MCreatorMyticStone.block.getDefaultState()
							.getBlock()) && (ground2 == MCreatorMyticSand.block.getDefaultState().getBlock() || ground2 == MCreatorMyticStone.block
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == MCreatorMyticSand.block.getDefaultState().getBlock()
